...
type (
	...
	Groups struct {
		...
		Status          int64          `db:"status"`           // 群状态：1-正常 2-解散 3-禁言
		CreatorUid      string         `db:"creator_uid"`      // 群创建者ID（关联users.id）
		GroupType       int64          `db:"group_type"`       // 群类型：1-普通群 2-企业群 3-兴趣群
		IsVerify        int64          `db:"is_verify"`        // 加群是否验证：0-否 1-是
		Notification    string         `db:"notification"`     // 群公告
		NotificationUid sql.NullString `db:"notification_uid"` // 发布公告的用户ID
