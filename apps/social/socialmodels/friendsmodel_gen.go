...
var (
	...
	friendsRowsExpectAutoSet   = strings.Join(stringx.Remove(friendsFieldNames, "`create_at`", "`create_time`", "`created_at`", "`update_at`", "`update_time`", "`updated_at`"), ",")
...
type (
	friendsModel interface {
		...
		FindOne(ctx context.Context, id string) (*Friends, error)
		...
		FindByUserId(ctx context.Context, userId string) ([]*Friends, error)
	}
	...
	Friends struct {
		Id        string       `db:"id"`
		UserId    string       `db:"user_id"`    // 当前用户ID（关联users.id）
		FriendUid string       `db:"friend_uid"` // 好友ID（关联users.id）
		Remark    string       `db:"remark"`     // 好友备注
		AddSource int64        `db:"add_source"` // 添加来源：1-搜索 2-群聊 3-推荐
...
func (m *defaultFriendsModel) Delete(ctx context.Context, id string) error {
...
func (m *defaultFriendsModel) FindOne(ctx context.Context, id string) (*Friends, error) {
	friendsIdKey := fmt.Sprintf("%s%v", cacheFriendsIdPrefix, id)
	var resp Friends
...
func (m *defaultFriendsModel) Insert(ctx context.Context, data *Friends) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Id, data.UserId, data.FriendUid, data.Remark, data.AddSource)
	}, friendsIdKey, friendsUserIdFriendUidKey)
	return ret, err
}
...
func (m *defaultFriendsModel) Inserts(ctx context.Context, session sqlx.Session, data ...*Friends) (sql.Result, error) {
	...
	for i, v := range data {
		sql.WriteString("(?,?,?,?,?)")
		args = append(args, v.Id, v.UserId, v.FriendUid, v.Remark, v.AddSource)
		if i == len(data)-1 {
			break
		}
		sql.WriteString(",")
...
func (m *defaultFriendsModel) FindByUserId(ctx context.Context, userId string) ([]*Friends, error) {
	query := fmt.Sprintf("select %v from %v where `user_id`=?", friendsRows, m.table)
	friends := []*Friends{}
	err := m.QueryRowsNoCacheCtx(ctx, &friends, query, userId)
	switch err {
	case nil:
		return friends, nil
